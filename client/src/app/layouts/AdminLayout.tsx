  ...
  return (
    <div className="flex h-full flex-col bg-navy-950 text-white">
      <div className="flex h-20 items-center border-b border-white/10 px-6">
        <img src={logo} alt="PlainB" className="h-9 brightness-0 invert" />
      </div>
      <nav className="flex-1 space-y-1 p-4">
        {navigation.map(([label, to, Icon]) => (
          <NavLink
            ...
            className={({ isActive }) =>
              cn(
                'flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-bold text-navy-300 hover:bg-white/10 hover:text-white',
                isActive && 'bg-brand-500 text-navy-950 hover:bg-brand-500 hover:text-navy-950',
              )
            }
          >
      ...
      <div className="border-t border-white/10 p-4 text-xs text-navy-400">
        <BarChart3 className="mb-2 size-5 text-brand-400" />
        PlainB administration
      </div>
    </div>
  );
}
...
export function AdminLayout() {
  ...
  const revenueTrends = useAdminStore((state) => state.revenueTrends);
  ...
  useEffect(() => {
    ...
    void revenueTrends();
    void loadCatalog();
  }, [load, revenueTrends, loadCatalog]);
        ...
        <header className="sticky top-0 z-30 flex h-20 items-center gap-4 border-b bg-white/95 px-4 backdrop-blur sm:px-7">
          <Dialog.Root>
            ...
            <Dialog.Portal>
              ...
              <Dialog.Content className="fixed inset-y-0 left-0 z-50 w-72">
          ...
          <div className="flex-1">
            ...
            <h1 className="text-xl font-black">{page}</h1>
          </div>
