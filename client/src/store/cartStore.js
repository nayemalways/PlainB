...
const CartStore = create((set) => ({
    ...
    SaveCartRequest: async (postbody, productID, quantity) => {
        try {
            ...
            let res = await axios.post(`${BaseServerUrl}/api/SaveProductToCart`, postbody, config); // Api call
    ...
    CartListRequest: async () => {
        try {
            let config = { headers: { token: Cookies.get('token')}}; // Ensure user logged in
            let res = await axios.get(`${BaseServerUrl}/api/SelectCartListProduct`, config);
    ...
    removeCartProduct: async (productID) => {
        try {
            ...
            let res = await axios.post(`${BaseServerUrl}/api/RemoveProductFromCart`, postBody, config);
            return res.data["status"] === "Success" ? true : res.data["message"];
        }catch(e) {
            unauthorized(e.response.status);
            console.log(e.toString());
    ...
    createInvoice: async () => {
        try {
            set({isChekout: true});
            let config = { headers: { token: Cookies.get('token')}}; // Ensure user logged in
            let res = await axios.get(`${BaseServerUrl}/api/CreateInvoice`, config);
            set({isChekout: true});
